...
type TabId = 'overview' | 'confidence' | 'agreement' | 'coverage' | 'evidence' | 'details' | 'disagreements'
...
const tabs: { id: TabId; name: string }[] = [
  ...
  { id: 'evidence', name: 'Evidence Quality' },  // Sprint 11d
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Evidence Quality</th>
            ...
            {modelKeys.map(modelKey => {
              ...
              return (
                <tr key={modelKey} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    {stats.avgEvidenceQuality !== undefined ? (
                      <span className={`font-medium ${getConfidenceColor(stats.avgEvidenceQuality)}`}>
                        {(stats.avgEvidenceQuality * 100).toFixed(0)}%
                      </span>
                    ) : (
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-gray-600">
                    {stats.cost?.totalCost !== undefined ? (
                      <span className="font-medium">${stats.cost.totalCost.toFixed(4)}</span>
                    ) : (
                      <span className="text-gray-400">-</span>
                    )}
                  </td>
...
function DisagreementsTab({
  ...
  // Note: modelKeys is required by interface but results.keys() used instead
