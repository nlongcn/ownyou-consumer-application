/**
 * StageIndicator - Visual progress indicator for 3-stage A/B testing workflow
 *
 * Ported from admin-dashboard with minimal changes.
 */

import type { StageStatus } from '@ownyou/ab-testing';
...
const stages = [
  ...
  { num: 2, name: 'Summarize', key: 'preprocess' as const },
  { num: 3, name: 'Classify', key: 'classify' as const },
];
    ...
    <div className="flex items-center justify-center mb-6">
      {stages.map((stage, index) => (
        <div key={stage.num} className="flex items-center">
          {/* Connector line */}
          {index > 0 && (
            <div className={`w-12 h-1 ${getConnectorColor(stage.num)}`} />
          )}

          {/* Stage circle and label */}
          <div className="flex flex-col items-center">
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold text-sm ${getStageColor(
                stage.num,
