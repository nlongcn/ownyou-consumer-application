...
def extract_purchase_with_agent(
    ...
    Example:
        ...
        >>> emails = [{{"subject": "...", "body": "..."}}]
        >>> result = extract_purchase_with_agent(emails, llm_client)
    """
    from .tools import (
    ...
    taxonomy_context = get_cached_taxonomy_context("purchase")
    ...
    system_prompt_with_taxonomy = f"""{PURCHASE_AGENT_SYSTEM_PROMPT}
...
- Do NOT include the full tier path in the value field"""
    ...
    user_prompt = PURCHASE_AGENT_USER_PROMPT.format(
    ...
    try:
        for iteration in range(max_iterations):
            iterations += 1
            logger.info(f"Purchase agent iteration {iteration + 1}/{max_iterations}")
            ...
            logger.info(f"📝 Purchase LLM Response: {len(parsed_classifications)} classifications returned")
            ...
            for classification in parsed_classifications:
                ...
                if validation_result.get("valid"):
                    # Ensure purchase_intent_flag is present
                    if "purchase_intent_flag" not in classification:
                        classification["purchase_intent_flag"] = "PIPR_LOW"  # Default
            ...
            if taxonomy_valid_classifications:
                ...
                from ..workflow.prompts import PURCHASE_EVIDENCE_GUIDELINES
                ...
                for classification, evidence_eval in zip(taxonomy_valid_classifications, evidence_evals):
                    ...
                    if will_block:
                        logger.warning(
                            f"Blocked inappropriate purchase intent inference: {classification.get('value')} "
        ...
        logger.error(f"Purchase agent error: {e}", exc_info=True)
        return {
            "classifications": [],
            "iterations": iterations,
            "tool_calls": tool_calls,
