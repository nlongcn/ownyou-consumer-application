...
def extract_demographics_with_agent(
    ...
    Example:
        ...
        >>> emails = [{"subject": "...", "body": "..."}]
        >>> result = extract_demographics_with_agent(emails, llm_client)
    """
    from .tools import (
    ...
    taxonomy_context = get_cached_taxonomy_context("demographics")
    ...
    system_prompt_with_taxonomy = f"""{DEMOGRAPHICS_AGENT_SYSTEM_PROMPT}
...
- Do NOT include the full tier path in the value field"""
    ...
    try:
        for iteration in range(max_iterations):
            iterations += 1
            logger.info(f"Demographics agent iteration {iteration + 1}/{max_iterations}")
            ...
            logger.info(f"📝 Demographics LLM Response: {len(parsed_classifications)} classifications returned")
            ...
            if taxonomy_valid_classifications:
                ...
                from ..workflow.prompts import DEMOGRAPHICS_EVIDENCE_GUIDELINES
        ...
        logger.error(f"Demographics agent error: {e}", exc_info=True)
        return {
            "classifications": [],
            "iterations": iterations,
            "tool_calls": tool_calls,
