...
def extract_household_with_agent(
    ...
    Example:
        ...
        >>> emails = [{{"subject": "...", "body": "..."}}]
        >>> result = extract_household_with_agent(emails, llm_client)
    """
    from .tools import (
    ...
    taxonomy_context = get_cached_taxonomy_context("household")
    ...
    system_prompt_with_taxonomy = f"""{HOUSEHOLD_AGENT_SYSTEM_PROMPT}
...
- Example: For "ID 100: Household Data | Life Stage | Grandparents with Children", use taxonomy_id=100 and value="Grandparents with Children"
...
- Do NOT include the full tier path in the value field"""
    ...
    user_prompt = HOUSEHOLD_AGENT_USER_PROMPT.format(
    ...
    try:
        for iteration in range(max_iterations):
            iterations += 1
            logger.info(f"Household agent iteration {iteration + 1}/{max_iterations}")
            ...
            logger.info(f"📝 Household LLM Response: {len(parsed_classifications)} classifications returned")
            ...
            if taxonomy_valid_classifications:
                ...
                from ..workflow.prompts import HOUSEHOLD_EVIDENCE_GUIDELINES
        ...
        logger.error(f"Household agent error: {e}", exc_info=True)
        return {
            "classifications": [],
            "iterations": iterations,
            "tool_calls": tool_calls,
