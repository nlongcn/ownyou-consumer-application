...
// Compute stats for a single model
...
function getPrimaryClassification(
  ...
  const emailClassifications = classifications.filter(c => c.emailId === emailId)
...
export function computeAgreementMetrics(
  ...
  for (const emailId of emailIds) {
    ...
    for (const modelKey of modelKeys) {
      ...
      const primary = getPrimaryClassification(modelResults.classifications, emailId)
      if (primary) {
  ...
  const totalClassified = fullAgreementCount + partialAgreementCount + noAgreementCount
  ...
  for (let i = 0; i < modelKeys.length; i++) {
    ...
    for (let j = 0; j < modelKeys.length; j++) {
      ...
      for (const emailId of emailIds) {
        const primaryA = getPrimaryClassification(resultsA.classifications, emailId)
        const primaryB = getPrimaryClassification(resultsB.classifications, emailId)
      ...
      pairwiseAgreement[modelA][modelB] = comparisons > 0 ? agreements / comparisons : 0
    }
  }
  ...
  return {
    ...
    partialAgreementCount,
    noAgreementCount,
    agreementRate: totalClassified > 0 ? fullAgreementCount / totalClassified : 0,
    pairwiseAgreement,
  }
}
...
export function computeCoverageMetrics(
  ...
  const modelKeys = Array.from(results.keys())
  const commonCategories = Array.from(allCategories).filter(category =>
  ...
  for (const modelKey of modelKeys) {
    uniqueCategories[modelKey] = categoriesByModel[modelKey]?.filter(category =>
      modelKeys.filter(k => k !== modelKey).every(k =>
        !categoriesByModel[k]?.includes(category)
      )
    ).sort() || []
  }
