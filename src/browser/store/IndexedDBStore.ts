/**
 * IndexedDBStore: Production Implementation
 *
 * Browser-based persistent storage for LangGraph.js agents using IndexedDB.
 * Extends BaseStore from @langchain/langgraph-checkpoint to provide long-term
 * cross-agent memory for OwnYou Consumer Application.
 *
 * Use Cases:
 * - IAB Taxonomy classifications (persistent across sessions)
 * - Mission Cards (generated by Mission Agents)
 * - User preferences and settings
 * - Any data requiring cross-agent memory
 *
 * Architecture:
 * - **Single Source of Truth:** All persistent data flows through this Store
 * - **Self-Sovereign:** Data stays in browser, user owns and controls it
 * - **Privacy-First:** No external database, no cloud sync (MVP Phase 2-5)
 *
 * Validated:
 * - Research spike (January 2025) - 100% test pass rate
 * - Performance: <10ms operations, supports 50MB-1GB data
 * - Persistence: Data survives browser refresh/restart
 * - Isolation: Namespace-based multi-user support
 *
 ...
 *
 * // Search all classifications
 * const all = await store.search(["user_123", "iab_classifications"]);
 * ```
 */
export class IndexedDBStore extends BaseStore {
  ...
  /**
   * @param dbName Name of the IndexedDB database (default: "langgraph-store")
   */
   ...
   * Supports filtering by exact field values and comparison operators.
   *
   * Filter Operators:
   * - $eq: Equal
   * - $ne: Not equal
   ...
   *
   * @param namespace Hierarchical path for the item (e.g., ["user_123", "iab_classifications"])
   * @param key Unique identifier within the namespace (e.g., "taxonomy_25")
   * @param value Object containing the item's data
   * @param index Optional indexing configuration (reserved for future use)
   */
  async put(
    namespace: string[],
    key: string,
